...
class HybridRetrieverService:
    ...
    def search(
        ...
        # Default to hybrid
        if self.bm25_search:
            return self.hybrid_logic.search(query, top_k=top_k, fusion_method=fusion_method)
        else:
            logger.warning("BM25 not initialized for hybrid search, falling back to vector")
            return self.veda_retriever.search(query, top_k=top_k)

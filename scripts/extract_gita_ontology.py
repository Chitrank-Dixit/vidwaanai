...
ENTITIES_DICT = {
    "Krishna": {
        "type": "Deity",
        "keywords": [r"krishna", r"kṛṣṇa", r"vasudeva", r"madhusudana", r"hrishikesha", r"keshava", r"govinda", r"achyuta", r"madhava", r"कृष्ण", r"केशव", r"गोविन्द", r"माधव", r"वासुदेव", r"ऋषिकेश"],
        "description": "Avatara of Vishnu, chariot driver and teacher of Arjuna in the Bhagavad Gita."
    },
    "Arjuna": {
        "type": "Character",
        "keywords": [r"arjuna", r"partha", r"kaunteya", r"dhananjaya", r"savyasachin", r"gudakesha", r"अर्जुन", r"पार्थ", r"कौन्तेय", r"धनंजय"],
    ...
    "Kurukshetra": {
        "type": "Place",
        "keywords": [r"kurukshetra", r"kuru-kshetra", r"dharmakshetra", r"कुरुक्षेत्र", r"धर्मक्षेत्र"],
    ...
    "Vishwarupa": {
        "type": "Concept",
        "keywords": [r"vishwarupa", r"viśvarūpa", r"cosmic form", r"universal form", r"विश्वरूप"],
...
def extract_relationships_from_entities(entities_present: set) -> list:
    ...
    if "Krishna" in entities_present and "Arjuna" in entities_present:
        rels.append({
            ...
            "to": "Arjuna",
            "type": "RELATED_TO",
            "attributes": {"context": "Krishna acts as Arjuna's preceptor and charioteer on the battlefield."}
        })
    ...
    if "Arjuna" in entities_present and "Kurukshetra" in entities_present:
        rels.append({
            ...
            "to": "Kurukshetra",
            "type": "LOCATED_AT",
            "attributes": {"context": "Arjuna stands ready to fight at Kurukshetra."}
        })
    ...
    if "Krishna" in entities_present and "Kurukshetra" in entities_present:
        rels.append({
            ...
            "to": "Kurukshetra",
            "type": "LOCATED_AT",
            "attributes": {"context": "Krishna drives Arjuna's chariot at Kurukshetra."}
        })
    ...
    if "Duryodhana" in entities_present and "Dhritarashtra" in entities_present:
        rels.append({
            ...
            "to": "Dhritarashtra",
            "type": "RELATED_TO",
            "attributes": {"context": "Duryodhana is the eldest son of Dhritarashtra."}
        })
    ...
    if "Sanjaya" in entities_present and "Dhritarashtra" in entities_present:
        rels.append({
            ...
            "to": "Dhritarashtra",
            "type": "RELATED_TO",
            "attributes": {"context": "Sanjaya narrates the battle of Kurukshetra to the blind king Dhritarashtra."}
        })
    ...
    if "Svadharma" in entities_present and "Dharma" in entities_present:
        rels.append({
            ...
            "to": "Dharma",
            "type": "RELATED_TO",
            "attributes": {"context": "Svadharma is an individual's personal duty aligned with universal Dharma."}
        })
    ...
    if "Karma Yoga" in entities_present and "Karma" in entities_present:
        rels.append({
            ...
            "to": "Karma",
            "type": "RELATED_TO",
            "attributes": {"context": "Karma Yoga is practiced by performing actions (Karma) selflessly."}
        })
    ...
    if "Atman" in entities_present and "Brahman" in entities_present:
        rels.append({
            ...
            "to": "Brahman",
            "type": "RELATED_TO",
            "attributes": {"context": "The individual soul (Atman) is realized as non-different from the universal Brahman."}
        })
    ...
    if "Moksha" in entities_present and "Samsara" in entities_present:
        rels.append({
            ...
            "to": "Samsara",
            "type": "RELATED_TO",
            "attributes": {"context": "Moksha is liberation from the cycle of rebirth (Samsara)."}
        })
    ...
    if "Krishna" in entities_present and "Vishwarupa" in entities_present:
        rels.append({
            ...
            "to": "Vishwarupa",
            "type": "RELATED_TO",
            "attributes": {"context": "Krishna manifests his Vishwarupa form to Arjuna."}
        })

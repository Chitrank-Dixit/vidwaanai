...
def get_llm_client():
    ...
    if settings.llm_backend == "lmstudio":
        ...
        if not settings.OPENAI_API_KEY:
            logger.warning("OPENAI_API_KEY is not set in environment/settings. Attempting load anyway...")
...
def main():
    ...
    for batch_id in range(start_batch, end_batch + 1):
        ...
        try:
            ...
            logger.info(f"Successfully processed batch {batch_id}: {len(data['entities'])} entities, {len(data['relationships'])} relationships.")
        except Exception as e:
    ...
    for start, end, target_name in groups:
        ...
        if group_responses:
            ...
            logger.info(f"Saved merged response to: {target_path} (Contains batches {start} to {end})")
        else:
            logger.warning(f"No data available for group {start}-{end}, skipping {target_name}")

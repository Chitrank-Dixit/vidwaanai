...
ENTITIES_DICT = {
    "Krishna": {
        "type": "Deity",
        "keywords": [r"krishna", r"kṛṣṇa", r"vasudeva", r"keshava", r"govinda", r"madhava", r"कृष्ण", r"वासुदेव", r"केशव"],
    ...
    "Arjuna": {
        "type": "Character",
        "keywords": [r"arjuna", r"partha", r"kaunteya", r"dhananjaya", r"अर्जुन", r"पार्थ", r"कौन्तेय", r"धनंजय"],
    ...
    "Draupadi": {
        "type": "Character",
        "keywords": [r"draupadi", r"draupadī", r"krishnaa", r"panchali", r"द्रौपदी", r"पांचाली"],
...
def extract_relationships_from_entities(entities_present: set) -> list:
        ...
        for p2 in pandavas:
            if p1 != p2 and p1 in entities_present and p2 in entities_present:
                rels.append({
                    ...
                    "to": p2,
                    "type": "RELATED_TO",
                    "attributes": {"context": f"{p1} and {p2} are brothers (the Pandavas)."}
                })
    ...
    if "Duryodhana" in entities_present:
        ...
        if "Shakuni" in entities_present:
            rels.append({
                ...
                "to": "Shakuni",
                "type": "RELATED_TO",
                "attributes": {"context": "Shakuni is Duryodhana's maternal uncle and close advisor."}
            })
        if "Karna" in entities_present:
            rels.append({
                ...
                "to": "Karna",
                "type": "RELATED_TO",
                "attributes": {"context": "Karna is Duryodhana's closest friend and key ally."}
            })
    ...
    if "Arjuna" in entities_present and "Krishna" in entities_present:
        rels.append({
            ...
            "to": "Arjuna",
            "type": "RELATED_TO",
            "attributes": {"context": "Krishna acts as Arjuna's charioteer, guide, and spiritual teacher."}
        })

    # 8. Participation in Kurukshetra War
    all_warriors = pandavas + ["Duryodhana", "Bhishma", "Drona", "Karna", "Ashwatthama", "Dhrishtadyumna"]
    for w in all_warriors:
        if w in entities_present and "Kurukshetra War" in entities_present:
            rels.append({
                ...
                "attributes": {"context": f"{w} participated as a major combatant in the Kurukshetra War."}
            })
    ...
    for actor in ["Yudhishthira", "Duryodhana", "Shakuni"]:
        if actor in entities_present and "Game of Dice" in entities_present:
            rels.append({
                ...
                "to": "Game of Dice",
                "type": "PARTICIPATED_IN",
                "attributes": {"context": f"{actor} participated in the fateful Game of Dice."}
            })
    ...
    for p in pandavas + ["Duryodhana", "Dhritarashtra", "Bhishma"]:
        if p in entities_present and "Hastinapura" in entities_present:
            rels.append({
                ...
                "to": "Hastinapura",
                "type": "LOCATED_AT",
                "attributes": {"context": f"{p} resided at or ruled from Hastinapura."}
            })
    ...
    for p in pandavas:
        if p in entities_present and "Indraprastha" in entities_present:
            rels.append({
                ...
                "to": "Indraprastha",
                "type": "LOCATED_AT",
                "attributes": {"context": f"{p} founded and ruled from Indraprastha."}
            })

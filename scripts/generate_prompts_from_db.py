...
def generate_prompts(limit=20, offset=0, scripture_filter=None):
    ...
    if scripture_filter:
        ...
        target_verses = [
            v
            for v in verses
            if scripture_filter.lower() in v.get("scripture_name", "").lower()
        ]
        ...
        print(
            f"Found {total_verses} verses. Generating {num_batches} batches of size {limit}."
        )
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=20, help="Number of verses to fetch"
    )
    parser.add_argument("--offset", type=int, default=0, help="Offset for fetching")
    parser.add_argument(
        "--scripture", type=str, help="Filter by scripture name (e.g. 'Ramayan')"
    )

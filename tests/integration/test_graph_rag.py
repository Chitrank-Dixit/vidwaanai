...
def mock_agent_deps():
    with (
        patch("src.core.agent_service.DatabaseManager"),
        patch("src.core.agent_service.VedaEmbedder"),
        patch("src.core.agent_service.LMStudioClient"),
        patch("src.core.agent_service.GraphBuilder"),
        patch("src.core.agent_service.GraphRetriever"),
        ...
        mock_service.graph_retriever.format_subgraph_context.return_value = (
            "Agni --[IS_A]--> Deity"
        )
...
def test_process_query_with_graph_context(mock_agent_deps):
    ...
    assert "graph_retrieval" in [
        trace["action"] for trace in response["reasoning_trace"]
    ]
...
def test_process_query_no_entities(mock_agent_deps):
    ...
    agent.process_query("Hello", session_id="test")

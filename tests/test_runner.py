...
class VidwaanLanguageTestSuite:
    ...
    def load_prompts(self, language_code: str) -> List[Dict]:
        ...
        try:
            with open(prompt_file, 'r', encoding='utf-8') as f:
                ...
                logger.info(f"Loaded {len(self.prompts[language_code])} prompts for {language_code}")
    ...
    def run_test(self, prompt: Dict, language_code: str, attempt: int = 1) -> Dict:
        ...
        try:
            ...
            return self._create_error_result(prompt_id, language_code, "timeout", str(e))
        except Exception as e:
            if attempt < self.config.get('max_retries', 3):
                ...
                logger.error(f"Failed after {attempt} attempts for {prompt_id}: {str(e)}")
                return self._create_error_result(prompt_id, language_code, "error", str(e))
    
    def _call_vidwaan_api(self, prompt_text: str, language_code: str, timeout: int) -> Dict:
        """
        ...
        try:
            ...
            return {
                ...
                "confidence": response.get("confidence", {}).get("total_confidence", 0.0), # Assuming structure
                "detected_language": response.get("language", language_code),
                "relevance_score": response.get("confidence", {}).get("retrieval_score", 0.0),
                "factual_accuracy": response.get("confidence", {}).get("factual_consistency", 0.0),
    ...
    def _calculate_metrics(self, response: Dict, prompt: Dict, language_code: str, 
        ...
        keyword_precision = (
            len(found_keywords) / len(response_tokens) 
    ...
    def _create_error_result(self, prompt_id: str, language_code: str, 
    ...
    def run_all_tests(self, languages: Optional[List[str]] = None,
        ...
        for lang_code in target_languages:
            ...
            lang_entry = next((l for l in self.LANGUAGES if l['code'] == lang_code), None)
            ...
            logger.info(f"Completed {lang_code}: {len(self.results[lang_code])} prompts")
    ...
    def _calculate_summary_statistics(self) -> Dict:
        ...
        for lang_code, results in self.results.items():
            ...
            if successful:
                avg_f1 = sum(r['metrics']['keyword_f1_score'] for r in successful) / len(successful)
                avg_precision = sum(r['metrics']['keyword_precision'] for r in successful) / len(successful)
                avg_recall = sum(r['metrics']['keyword_recall'] for r in successful) / len(successful)
                avg_time = sum(r['metrics']['processing_time_ms'] for r in successful) / len(successful)
            else:
        ...
        def generate(self, prompt, max_tokens=None, temperature=None):
            ...
            if marker in prompt:
                try:
                    ...
                    context_part = prompt.split(marker, 1)[1].split("Please answer", 1)[0]
                    ...
                    words = clean_context.split()[:30] # increased word count to catch more keywords
...
def main():
    ...
    parser.add_argument('--languages', nargs='+', help='Languages to test (e.g., en hi bn)')
    parser.add_argument('--categories', nargs='+', help='Categories to test (e.g., ramayana gita)')
    parser.add_argument('--config', default='config/test_config.json', help='Config file path')
    parser.add_argument('--no-mock-llm', action='store_true', help='Disable Mock LLM and use real backend')
